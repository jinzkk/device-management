...
| 소스 관리 | GitHub (`jinzkk/device-management`) |
| 배포 | Vercel (프로젝트: `jinzkk/text-view`, URL: https://text-view.vercel.app) |
...
### 15단계: QR 코드 인쇄 CSS 최적화 -- ✅ 완료 (Playwright 검증 완료)
- [x] `src/app/equipment/layout.tsx` — Sidebar/Header를 `print:hidden` div로 감쌈 (인쇄 시 제거)
- [x] `src/components/equipment/qr-code-display.tsx` — `@page { margin: 10mm; }` 추가, 외부 wrapper에 `print:flex print:items-center print:justify-center print:min-h-screen`, 카드 패딩/텍스트 인쇄 최적화

### 16단계: 장비 사진 업로드 (Supabase Storage 연동) -- ✅ 완료 (Playwright 검증 완료)
- [x] Supabase Storage `equipment-images` Public 버킷 생성
- [x] Storage RLS 정책 추가 (authenticated 사용자 INSERT/UPDATE/DELETE)
- [x] `src/lib/validators/equipment.ts` — `image_url` 필드 추가
- [x] `src/components/equipment/equipment-form.tsx` — 파일 선택 UI, 미리보기 (w-32 h-32), 삭제 버튼, Storage 업로드 로직 (create/edit 모드 공통)
- [x] `src/app/equipment/[id]/page.tsx` — 장비 상세 카드 상단 이미지 표시
- [x] **버그 수정**: `src/app/equipment/[id]/loading.tsx` — `map()` 내 `<>` Fragment에 `key` 누락 → `<Fragment key={i}>` 로 수정 (콘솔 오류 해결)
- [x] **버그 수정**: `src/app/equipment/[id]/page.tsx` — 이미지 `w-full object-cover` → `object-contain + flex justify-center` 로 수정 (왜곡 해결, 비율 1.05 → 1.05 일치)
...
│   ├── qr.ts                              # QR URL 빌더
│   └── utils.ts                            # cn() 유틸리티
├── proxy.ts                                # 인증 미들웨어 (Next.js 16, middleware.ts 대체)
